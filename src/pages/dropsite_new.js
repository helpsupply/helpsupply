...
class NewDropSite extends React.Component {
  ...
  componentDidMount() {
    Promise.resolve(this.checkVerification()).then(() => {
      ...
      if (dropsite) {
        // TODO: redirect to facility page if it exists
  ...
  onSubmit(hospital) {
    this.props.backend
      .addNewDropSite(
        hospital.dropSiteName,
        hospital.dropSiteDescription,
        hospital.dropSiteAddress,
        hospital.dropSiteZip,
        hospital.dropSiteHospital,
        hospital.dropSitePhone
      )
      .then((data) => {
